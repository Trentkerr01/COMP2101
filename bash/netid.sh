...
if [ $# = 0 ]; then
	echo "No Arguments were put on command line"
elif [ $# = 1 ]; then
	if [ $1 = "-v" ]; then
		echo "Verbose has been enabled"
		verbose=$"yes"
	elif [ $1 = "ens33" ]; then
		interface=$"ens33"
		echo "The interface you have selected is $interface"
	else
		echo "I don't understand the argument"
	fi
elif [ $# = 2 ]; then
	if [ $1 = "-v" ]; then
		echo "Verbose has been enabled"
		...
		echo "I dont understand the arguments"
	fi
else
	echo "I dont understand the arguments"
fi
...
echo "List of Interfaces on Device"

listinterface=$(netstat -i | awk '{print $1}' | grep -vE '(Kernel|Iface|lo)')
listofinterfacenum=$( echo $listinterface | wc -w )
echo $listofinterfacenum


if [ '$interface' = '$listinterface' ]; then
...
while [ $count -lt $listofinterfacenum ]; do
